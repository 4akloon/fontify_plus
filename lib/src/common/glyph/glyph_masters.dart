...
/// and [max] — which is only possible because [glyphMastersFromSvg] built them
...
///
/// A path can be both filled and stroked, and the two are independent: the
/// fill is built once from the source commands and shared verbatim by both
/// masters, because a fill's area does not depend on stroke width. Each
...
/// then applied to both masters, so the two never make the classification
/// decision independently and drift apart.
///
...
GlyphMasters glyphMastersFromSvg(
  String name,
  String xmlString,
  ...
  checkMastersCompatible(name, minGlyph, maxGlyph);
...
void checkMastersCompatible(String glyphName, GenericGlyph a, GenericGlyph b) {
